...
- **`opcli spread run`/`opcli env provision` can't find/use LXD right after install** — joining the `lxd` group requires a new login session. Log out and back in (or `newgrp lxd`) and re-run `opcli install doctor` to confirm.
- **`opcli env provision` fails looking for `concierge.yaml`** — it isn't generated by `opcli artifacts init`/`opcli spread init`. Copy `examples/concierge.yaml` (or `examples/concierge-k8s.yaml` for a k8s charm) into your repo first — see [`concierge.yaml` contract](#conciergeyaml-contract).
...
Both paths below assume `opcli install all` has already been run (see [Installation](#installation)) and that your repo has a `concierge.yaml` (copy `examples/concierge.yaml` as a starting point if you don't have one yet — see [`concierge.yaml` contract](#conciergeyaml-contract)).
...
jobs:
  build:
    uses: canonical/charm-ci/.github/workflows/build-artifacts.yml@v1.0.0
    permissions:
      contents: read
      packages: write
      actions: read
